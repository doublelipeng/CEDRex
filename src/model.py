...
from src.utils import length_to_mask, PositionalEncoding

class LarksTransformer(nn.Module):
    def __init__(self, vocab_size, hidden_dim, num_class, num_head=3, num_layers=3, max_len=1000, dropout=0.1):
        super().__init__()
        ...
        encoder_layer = nn.TransformerEncoderLayer(hidden_dim, num_head, hidden_dim*4, dropout, norm_first=True)
        decoder_layer = nn.TransformerDecoderLayer(hidden_dim, num_head, hidden_dim*4, dropout, norm_first=True)
